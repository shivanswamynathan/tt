...
class MongoDBClient:
        ...
        try:
            ...
            for topic in topics:
                ...
                topic_details.append({
                    "topic": topic,  # Keep full topic for searches
                    "display_name": short_name,  # Shorter name for display
                    "chunk_count": count,
    ...
    def get_topic_content_chunks(self, topic: str) -> List[Dict[str, Any]]:
        ...
        try:
            cursor = self.collection.find(
                {"topic": {"$regex": topic, "$options": "i"}},  # Handle long topic names
                {"text": 1, "id": 1, "topic": 1, "_id": 0}     # Use "id" not "chunk_id"
            )
            
            chunks = list(cursor)
            return chunks  # Remove the complex chunking logic since your data is already chunked
        except Exception as e:
        ...
        try:
            ...
            try:
                cursor = self.collection.find(
                    ...
                    {"text": 1, "id": 1, "topic": 1, "score": {"$meta": "textScore"}}
