...
class DynamicRevisionAgent:
    ...
    async def continue_revision(self, session_id: str, user_query: Optional[str] = None) -> Dict[str, Any]:
        ...
        logger.info(f"continue_revision called: session_id={session_id}, query='{user_query}'") 
    ...
    async def _wants_to_end_session(self, user_input: str) -> bool:
        ...
        Consider it ending ONLY if they explicitly say:
        - "end session"
        - "stop learning" 
        - "finish session"
        - "I'm done"
        ...
        Do NOT consider simple responses like "yes", "no", "ok", "sure" as ending the session.
        These are just normal conversation responses.
        ...
        try:
            ...
            # More specific fallback - only explicit end phrases
            end_phrases = ["end session", "stop learning", "finish session", "I'm done", "quit learning"]
    ...
    async def _handle_user_question(self, session_state: SessionState, user_query: str) -> Dict[str, Any]:
        ...
        relevant_content = self.mongodb.get_topic_content(session_state.topic, limit=3)
        ...
        return {
            ...
            "is_session_complete": False,
            "sources": [chunk.get("id", "Unknown") for chunk in relevant_content]
        }
    ...
    async def _continue_revision_content(self, session_state: SessionState, user_query: str) -> Dict[str, Any]:
        ...
        session_data = self.mongodb.get_revision_session(session_state.session_id)
        conversation_history = session_data.get("conversation_history", []) if session_data else []
        
        # Generate response using your existing prompt system with conversation context
        response = await self._generate_response_from_prompt(
    ...
    async def continue_revision(self, session_id: str, user_query: Optional[str] = None) -> Dict[str, Any]:
        """Continue revision with full conversation memory"""
        ...
        # USE YOUR EXISTING LOGIC WITH CONVERSATION HISTORY
        if session_state.current_stage == "quiz" and user_query:
            response_data = await self._process_quiz_answers(session_state, user_query)
        elif user_query and await self._is_question(user_query):
            response_data = await self._handle_user_question(session_state, user_query)
        elif self._should_auto_quiz(session_state):
            response_data = await self._generate_auto_quiz(session_state)
        else:
    ...
    async def _generate_response_from_prompt(self, prompt: str, system_message: str, conversation_history: list = None) -> str:
        """Helper to generate response using LLM with conversation context"""
        
        messages = [SystemMessage(content=system_message)]
        
        # ADD CONVERSATION HISTORY IF PROVIDED
        if conversation_history:
            context = self._build_conversation_context(conversation_history[-5:])  # Last 5 turns
            enhanced_prompt = f"""
    ...
    def _build_conversation_context(self, conversation_history: list) -> str:
        ...
        for turn in conversation_history:
            if turn.get("user_message"):
                context_lines.append(f"Student: {turn['user_message']}")
            if turn.get("assistant_message"):
                context_lines.append(f"Tutor: {turn['assistant_message'][:200]}...")  # Truncate long responses
        return "\n".join(context_lines)
    
    def _extract_concept_name(self, text: str) -> str:
        """Extract concept name from text"""
        # Take first sentence or first 50 characters as concept name
    ...
    async def _get_or_restore_session(self, session_id: str) -> Optional[SessionState]:
        ...
        logger.info(f"Session not in memory, checking MongoDB...")
        
        session_data = self.mongodb.get_revision_session(session_id)
        if session_data:
            logger.info(f"Found session in MongoDB, restoring...")
