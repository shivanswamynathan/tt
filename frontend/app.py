...
def show_revision_interface():
    ...
    st_autorefresh(interval=1500, key="chat-refresh") 
    ...
    if hasattr(st.session_state, 'pending_messages') and st.session_state.pending_messages:
        for msg in st.session_state.pending_messages:
            ...
            st.session_state.conversation_count = msg["metadata"].get(
                "conversation_count",
                st.session_state.conversation_count
            )
            # mark session complete if backend says so
            if msg["metadata"].get("is_session_complete"):
        ...
        def on_message(ws, message):
            ...
            try:
                ...
                if data["type"] == "message":
                    # Store in pending messages instead of direct access
                    ...
                    # DON'T call st.rerun() from here - it causes issues in WebSocket threads
...
def handle_user_input(user_input):
    ...
    try:
        ...
        # Rerun to show both user and assistant messages
        st.rerun()
